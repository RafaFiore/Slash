...
void ABaseCharacter::DirectionalHitReaction(const FVector& ImpactPoint)
{
	...
	LastHit = ELastHit::ELH_FromBackward;

	if (Theta >= -45.f && Theta < 45.f)
	{
		Section = FName("FromFront");
		LastHit = ELastHit::ELH_FromForward;
	}
	else if (Theta >= -135.f && Theta < -45.f)
	{
		Section = FName("FromLeft");
		LastHit = ELastHit::ELH_FromLeft;
	}
	else if (Theta >= 45.f && Theta < 135.f)
	{
		Section = FName("FromRight");
		LastHit = ELastHit::ELH_FromRight;
	}

	PlayHitReactMontage(Section);
}

void ABaseCharacter::DirectionalDeathMontage()
{
	FName SectionName = FName();
	switch (LastHit)
	{
	case ELastHit::ELH_FromBackward:
		SectionName = FName("DeathForward");
		DeathPose = EDeathPose::EDP_DeathForward;
		break;
	case ELastHit::ELH_FromLeft:
		SectionName = FName("DeathRight");
		DeathPose = EDeathPose::EDP_DeathRight;
		break;
	case ELastHit::ELH_FromRight:
		SectionName = FName("DeathLeft");
		DeathPose = EDeathPose::EDP_DeathLeft;
		break;
	case ELastHit::ELH_FromForward:
